...
class DoctrineListRepresentationFactory
{
     ...
     */
    private function addHeader(array $articles, string $locale): array
    {
        $ids = \array_filter(\array_column($articles, 'header'));
        $avatars = $this->mediaManager->getFormatUrls($ids, $locale);
        foreach ($articles as $key => $article) {
            if (\array_key_exists('header', $article)
                && $article['header']
                && \array_key_exists($article['header'], $avatars)
            ) {
                $articles[$key]['header'] = $avatars[$article['header']];
            }
        }
